 ...
 *
 * - fixed bug where navigation is not showing if navPosition was right
 *
 * */
...
;(function($, window, document) {
	...
	function setupCanny() {
		...
		if(_self.options.isResponsive == true) {
			...
			/*var addClass = debounce(function() {

			}, 250);
			window.addEventListener('resize', addClass);*/
		} else if(_self.options.isResponsive == false) {
			_navi.addClass('canny');
		}
		...
		if(_self.options.navPosition == 'left') {
			_navi
				...
				.css('left', '-' + (_navi.data('width') - _self.options.navOffset) + 'px');
		} else if(_self.options.navPosition == 'right') {
			_navi
				.addClass('canny-align-right')
				.css('right', '-' + (_navi.data('width') - _self.options.navOffset) + 'px');
		}
